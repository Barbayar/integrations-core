...
def test_default(aggregator, dd_run_check, mock_http_response):
    ...
    check = get_check({'prometheus_endpoints': ['test'], 'metrics': ['.+']})
    dd_run_check(check)
...
def test_disable_histogram_buckets(aggregator, dd_run_check, mock_http_response):
    payload = """
        ...
        """
    mock_http_response(payload)
    check = get_check({'prometheus_endpoints': ['test'], 'metrics': ['.+'], 'collect_histogram_buckets': False})
    dd_run_check(check)
...
def test_non_cumulative_histogram_buckets(aggregator, dd_run_check, mock_http_response):
    payload = """
        ...
        """
    mock_http_response(payload)
    check = get_check({'prometheus_endpoints': ['test'], 'metrics': ['.+'], 'non_cumulative_histogram_buckets': True})
    dd_run_check(check)
