...
def decumulate_histogram_buckets(sample_data):
    ...
    for sample, tags, hostname in new_sample_data:
        if not sample.name.endswith('_bucket'):
            ...
            tags.append('lower_bound:{}'.format(lower_bound))
...
def get_label_normalizer(metric_type):
    if metric_type == 'histogram':
        return normalize_histogram_labels
    elif metric_type == 'summary':
        return normalize_summary_labels
    else:
